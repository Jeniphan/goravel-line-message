...
func init() {
	...
	config.Add("database", map[string]any{
		...
		"connections": map[string]any{
			...
			// 	"driver":   "mysql",
			// 	"host":     config.Env("DB_HOST", "192.168.1.64"),
			// 	"port":     config.Env("DB_PORT", 3306),
			// 	"database": config.Env("DB_DATABASE", "my_website"),
			// 	"username": config.Env("DB_USERNAME", "akiira"),
			// 	"password": config.Env("DB_PASSWORD", "jeni78547854"),
			// 	"charset":  "utf8mb4",
			// 	// "loc":      "Local",
			// 	"prefix":   "",
			...
			"postgresql": map[string]any{
				...
				"database": config.Env("DB_DATABASE", "line_log"),
				"username": config.Env("DB_USERNAME", "postgres"),
				"password": config.Env("DB_PASSWORD", "akiira"),
				"sslmode":  "disable",
